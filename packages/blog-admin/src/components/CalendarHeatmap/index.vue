...
export default /*#__PURE__*/ defineComponent({
  ...
  setup(props) {
    ...
    function initTippy() {
      if (svg.value) {
        tippyInstances = tippy(
          Array.from(
            svg.value.querySelectorAll(".vch__day__square[data-tippy-content]")
          )
        );
        if (tippySingleton) {
    ...
    function safeDestroyTippy() {
      try {
        if (tippySingleton && !tippySingleton.state?.isDestroyed) {
          tippySingleton.destroy();
          tippySingleton = null as any;
        }

        if (tippyInstances && tippyInstances.length) {
        ...
        console.error("销毁tippy实例时出错:", error);
      }
    }
        ...
        try {
          ...
          nextTick(() => {
            try {
              ...
              console.error("重新初始化tippy失败:", error);
            }
          });
        } catch (error) {
          console.error("更新热图失败:", error);
        }
      }
    );
    ...
    onMounted(() => {
      try {
        ...
        console.error("挂载时初始化tippy失败:", error);
      }
    });
    ...
    onBeforeUnmount(() => {
      try {
        ...
        console.error("卸载时清理tippy失败:", error);
      }
    });
