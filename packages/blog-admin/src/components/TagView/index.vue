...
const closeSelectedTag = (view: TagView) => {
  tagStore.delView(view).then((res: any) => {
    if (isActive(view)) {
...
const closeOtherTags = () => {
  ...
  tagStore.delOtherViews(selectedTag.value).then(() => {
    moveToCurrentTag();
  });
  closeMenu();
};
const closeAllTags = (view: TagView) => {
  tagStore.delAllViews().then((res: any) => {
    toLastView(res.visitedViews, view);
  });
  closeMenu();
};
  ...
  try {
    return (
      (selectedTag.value as TagView).fullPath ===
        tagStore.visitedViews[1].fullPath ||
...
const isLastView = () => {
  try {
    return (
      (selectedTag.value as TagView).fullPath ===
      tagStore.visitedViews[tagStore.visitedViews.length - 1].fullPath
    );
  } catch (err) {
    return false;
  }
...
const closeLeftTags = () => {
  tagStore.delLeftViews(selectedTag.value).then((res: any) => {
    if (
...
const closeRightTags = () => {
  tagStore.delRightViews(selectedTag.value).then((res: any) => {
    if (
      !res.visitedViews.find((item: any) => item.fullPath === route.fullPath)
    ) {
      toLastView(res.visitedViews);
