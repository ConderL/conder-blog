...
const handleCheckedTreeNodeAll = () => {
  treeRef.value!.setCheckedNodes(
    menuNodeAll.value ? (treeData.value as Node[]) : []
  );
};
...
const handleChangeStatus = (role: Role) => {
  ...
  messageConfirm("确定要" + text + role.roleName + "角色吗?")
    .then(() => {
      updateRoleStatus({ id: role.id, isDisable: role.isDisable }).then(
        ({ data }) => {
          if (data.flag) {
            notifySuccess(data.msg);
          } else {
