<template>
  <div class="app-container">
    ...
    <el-empty
      v-if="talkList === null || talkList.length === 0"
      description="暂无说说"
    ></el-empty>
    <!-- 说说列表 -->
    <div class="talk-item" v-for="talk of talkList" :key="talk.id">
      ...
      <div class="talk-content-container">
        ...
        <div class="user-info">
          <!-- 用户昵称 -->
          <div class="user-name">{{ talk.user?.nickname || "" }}</div>
          <!-- 操作 -->
        ...
        <div class="talk-info">
          ...
          <span class="top" v-if="talk.isTop === 1"
            ><svg-icon icon-class="top" size="0.85rem"></svg-icon>置顶</span
          >
        ...
        <el-row
          ...
          v-if="talk.imgList && talk.imgList.length > 0"
        >
          <el-col
            :md="8"
            :cols="6"
...
const handleOperation = (command: string) => {
  ...
  talkForm.value.id = Number.isNaN(id) ? undefined : id;
  ...
  if (type === "delete") {
    ...
    editTalk(talkForm.value.id!).then(({ data }) => {
      if (data.flag) {
        ...
        uploadList.value = [];
        // 确保imgList存在且是数组
        if (data.data.imgList && Array.isArray(data.data.imgList)) {
...
const getList = () => {
  previewList.value = []; // 清空预览列表，防止重复添加
  getTalkList(queryParams.value)
    .then(({ data }) => {
      if (data.flag && data.data) {
        // 安全地设置记录列表，确保每条记录都有正确的结构
        if (data.data.recordList && Array.isArray(data.data.recordList)) {
          talkList.value = data.data.recordList.map((item) => {
            // 确保user对象存在
            if (!item.user) {
              item.user = { avatar: "", nickname: "" };
            }
            // 确保imgList是数组
            if (!item.imgList) {
              item.imgList = [];
        ...
        talkList.value = [];
        count.value = 0;
        console.warn("获取说说列表失败:", data.msg);
      }
    })
    .catch((error) => {
      talkList.value = [];
      count.value = 0;
      console.error("获取说说列表出错:", error);
    });
};
onMounted(() => {
  getList();
