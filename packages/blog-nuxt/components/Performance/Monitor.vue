<template>
  <div v-if="showMonitor" class="performance-monitor">
    <div class="monitor-header">
      <h3>性能监控</h3>
      <UButton size="xs" @click="showMonitor = false">关闭</UButton>
    </div>
...
onMounted(() => {
  ...
  if (process.client) {
    ...
    window.showPerformanceMonitor = () => {
      ...
      console.log('%c性能监控面板已显示', 'color: #41b883; font-weight: bold');
    };
    
    window.hidePerformanceMonitor = () => {
      showMonitor.value = false;
      console.log('%c性能监控面板已隐藏', 'color: #41b883; font-weight: bold');
    };
    
    // 输出使用说明
    console.log('%c要显示性能监控面板，请在控制台执行: showPerformanceMonitor()', 'color: #41b883');
  }
});
...
function collectBasicMetrics() {
  ...
  if (timing.loadEventEnd > 0) {
    ...
    // 输出到控制台
    console.log('%c===== Nuxt SSR 性能指标 =====', 'color: #41b883; font-weight: bold');
...
function collectWebVitals() {
  ...
  onCLS(metric => {
    ...
    console.log(`%c[Nuxt SSR] CLS: ${metric.value.toFixed(3)}`, 'color: #41b883');
  });
  
  // 首次内容绘制 (FCP)
  onFCP(metric => {
    metrics.fcp = Math.round(metric.value);
    console.log(`%c[Nuxt SSR] FCP: ${metric.value.toFixed(2)}ms`, 'color: #41b883');
  });
  
  // 交互到下一次绘制 (INP)
  onINP(metric => {
    metrics.inp = Math.round(metric.value);
    console.log(`%c[Nuxt SSR] INP: ${metric.value.toFixed(2)}ms`, 'color: #41b883');
  });
  
  // 最大内容绘制 (LCP)
  onLCP(metric => {
    metrics.lcp = Math.round(metric.value);
    console.log(`%c[Nuxt SSR] LCP: ${metric.value.toFixed(2)}ms`, 'color: #41b883');
  });
  
  // 首字节时间 (TTFB)
  onTTFB(metric => {
    metrics.ttfb = Math.round(metric.value);
    console.log(`%c[Nuxt SSR] TTFB: ${metric.value.toFixed(2)}ms`, 'color: #41b883');
  });
}
</script>
